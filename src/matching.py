...
def match_all(df_professional, df_resposta,df_matchings)-> pd.DataFrame:
    ...
    df_professional.to_csv("./csv/professional1.csv", index=False)
    df_resposta.to_csv("./csv/resposta1.csv", index=False)
    query = """
    SELECT * FROM(SELECT 
        paci.name_paciente,
        paci.area AS area,
    ...
    match_all.to_csv("./csv/match_all.csv", index=False)
    return match_all
...
def match_previous(df_match_all, df_matchings)-> pd.DataFrame:
    ...
    df_selected_matches.to_csv("./csv/match_previous.csv", index=False)
...
def match(df_professional, df_resposta, df_matchings, save=False)-> pd.DataFrame:
    ...
    df_selected_matches.to_csv("./csv/matchings_selected.csv", index=False)
